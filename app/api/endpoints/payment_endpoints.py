...
async def create_payment(payment_data: payment_schema.PaymentSchema, db: DBSession):
...
async def get_all_payments(db: DBSession):
...
async def get_payment(payment_id: str, db: DBSession):
...
async def get_payment_by_reference(payment_reference: str, db: DBSession):
...
async def get_payments_by_order(order_id: str, db: DBSession):
...
async def update_payment(payment_id: str, payment_data: payment_schema.PaymentSchema, db: DBSession):
    return await payment_service.update_payment(db, payment_id, payment_data)


@payment_router.delete("/{payment_id}", status_code=204)
async def delete_payment(payment_id: str, db: DBSession):
