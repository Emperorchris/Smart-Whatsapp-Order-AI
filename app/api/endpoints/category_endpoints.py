...
async def create_category(data: category_schema.CategorySchema, db: DBSession):
...
async def get_root_categories(db: DBSession):
...
async def get_category(category_id: str, db: DBSession):
...
async def get_subcategories(category_id: str, db: DBSession):
...
async def update_category(category_id: str, data: category_schema.CategorySchema, db: DBSession):
    return await category_service.update_category(db, category_id, data)


@category_router.delete("/{category_id}", status_code=204)
async def delete_category(category_id: str, db: DBSession):
