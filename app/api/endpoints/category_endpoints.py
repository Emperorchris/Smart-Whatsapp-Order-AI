...
async def create_category(data: category_schema.CategorySchema, db: DBSession, _: AdminOnly):
...
async def get_all_categories(db: DBSession, _: CurrentStaff):
...
async def get_root_categories(db: DBSession, _: CurrentStaff):
...
async def get_category(category_id: str, db: DBSession, _: CurrentStaff):
...
async def get_subcategories(category_id: str, db: DBSession, _: CurrentStaff):
    return await category_service.get_subcategories(db, category_id)


@category_router.put("/{category_id}", response_model=category_schema.CategoryResponse)
async def update_category(category_id: str, data: category_schema.CategorySchema, db: DBSession, _: AdminOnly):
    return await category_service.update_category(db, category_id, data)


@category_router.delete("/{category_id}", status_code=204)
async def delete_category(category_id: str, db: DBSession, _: AdminOnly):
