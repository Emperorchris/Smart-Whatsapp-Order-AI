...
async def create_address(address_data: customer_address_schema.CustomerAddressSchema, db: DBSession):
...
async def get_addresses_by_customer(customer_id: str, db: DBSession):
...
async def get_default_address(customer_id: str, db: DBSession):
...
async def get_address(address_id: str, db: DBSession):
...
async def update_address(address_id: str, address_data: customer_address_schema.CustomerAddressSchema, db: DBSession):
...
async def set_default_address(address_id: str, db: DBSession):
...
async def delete_address(address_id: str, db: DBSession):
