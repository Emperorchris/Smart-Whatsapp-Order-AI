...
async def get_all_settings(db: DBSession, _: CurrentStaff):
...
async def get_setting(key: str, db: DBSession, _: CurrentStaff):
...
async def upsert_setting(data: store_settings_schema.StoreSettingSchema, db: DBSession, _: AdminOnly):
...
async def bulk_upsert(data: store_settings_schema.StoreSettingsBulkUpdate, db: DBSession, _: AdminOnly):
    return await store_settings_service.bulk_upsert(db, data)


@settings_router.delete("/{key}", status_code=204)
async def delete_setting(key: str, db: DBSession, _: AdminOnly):
